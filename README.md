...
> Each stage can run as a named Kedro pipeline. For example, `data_drifts`
...
Metrics in `data/08_reporting/model_metrics.json` show RMSE 1.45, R2 0.954, and MAPE 1.57 %, with 86.8 % of predictions within +/- 2 points. `data/08_reporting/model_selection_report.json` covers the 5-fold CV that picked HistGradientBoosting, and `data/08_reporting/robustness_report.json` checks leakage risk with a player-grouped split (RMSE 1.54, no player overlap).
...
| `data_feat_engineering`  | `03_primary` -> `04_feature`            | Derived features plus feature-store metadata (the offline store)    |
...
pip install -e .            # installs the package and declared dependencies
```

## 4. Run the pipelines

```bash
# Default training run (quality, cleaning, FE, split, selection, train)
kedro run
...
# Useful macro-stages
kedro run --pipeline data_processing   # quality + cleaning + FE
kedro run --pipeline modelling         # selection + train`nkedro run --pipeline inference         # requires persisted model/split artefacts
```

Artefacts land under `data/` along the numbered layers. Metrics and SHAP plots
appear in `data/08_reporting/`; drift reports are generated by the `data_drifts` or `inference` pipelines.
...
Example request (raw column names; saved medians cover missing skill attributes listed in `data/06_models/serving_metadata.json`, not arbitrary omitted request fields):
```bash
...
of the actual value (see `data/07_model_output/predictions.csv`). The reporting artefacts include
goalkeeper/outfield and rating-band error slices, but not a protected-attribute fairness audit.

## 6. Tests

```bash
pytest                 # unit and pipeline tests (30 tests)
...
|---------------------|-------------------------|
| Unit data tests plus feature store | `data_quality` asserts; `data_feat_engineering` builds the feature table and metadata in `04_feature`. |
