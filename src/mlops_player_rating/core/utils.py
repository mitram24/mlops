"""Shared transformations used by Kedro pipelines and the serving API."""
...
def _num(df: pd.DataFrame, col: str) -> pd.Series:
    """Numeric view of ``col``. Missing columns become NaN series."""
...
def clean_work_rate(series: pd.Series) -> pd.Series:
    """Normalise a work-rate column to {low, medium, high}."""
...
def apply_value_semantics(df: pd.DataFrame) -> pd.DataFrame:
    """Resolve known dirty-value issues in work rates and preferred foot."""
    df = df.copy()
...
def fit_attribute_imputer(df: pd.DataFrame) -> dict[str, float]:
    """Fit median values for raw FIFA skill columns present in ``df``."""
    medians: dict[str, float] = {}
    for col in SKILL_COLUMNS:
        if col in df.columns:
            value = pd.to_numeric(df[col], errors="coerce").median()
            if pd.notna(value):
                medians[col] = float(value)
    return medians


def apply_attribute_imputer(df: pd.DataFrame, medians: dict[str, float]) -> pd.DataFrame:
    """Fill missing skill values and create omitted skill columns from saved medians."""
    df = df.copy()
...
def _compute_age(df: pd.DataFrame) -> pd.Series:
    """Age in years at the time of the FIFA snapshot."""
...
def drop_non_features(df: pd.DataFrame, extra: list[str] | None = None) -> pd.DataFrame:
    """Drop identifier, leakage and display-only columns."""
...
def preprocess_for_inference(
    ...
    """Transform raw serving records into the exact training feature schema."""
