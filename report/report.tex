...
\begin{tikzpicture}[
  ...
  \node[box, right=of c] (fe) {data\_feat\_engineering};
...
the risk of training-serving skew. Saved medians cover the skill attributes stored in the serving
metadata, but the API should not be described as accepting arbitrary omitted request fields. A row-level parity test would make this check explicit by
...
\label{sec:modelling}
We evaluated four models using 5-fold cross-validation. Table~\ref{tab:cv} shows that the tree-based models achieved lower RMSE than the linear baselines, suggesting that the linear models did not capture the relationship between attributes and rating as well. We selected HistGradientBoosting as the champion model. On the held-out test set, it achieved an RMSE of 1.45, MAE of 1.05, and an $R^2$ of 0.954.
...
The high $R^2$ is expected because the features include the same types of skill attributes used to assign FIFA ratings. However, because the dataset contains multiple snapshots per player, a standard random split can leak player identity between train and test sets. When we performed a grouped split on \code{player\_api\_id} to separate players across splits, performance changed only modestly (RMSE 1.54, $R^2$ 0.950, 84.2\% within 2 points), with zero overlapping players between train and test. This suggests that player overlap was not the only reason for the strong test result. The robustness report includes goalkeeper/outfield and rating-band error slices, but it does not include protected-attribute subgroup or fairness metrics.

Diagnostic plots (Figure~\ref{fig:diag}) show that predictions track observed ratings through most of the distribution, and the residual plot does not show a clear pattern.
...
The SHAP summary plot shows that \code{reactions} and \code{ball\_control} have the largest contributions, consistent with their high correlations in the initial EDA. The \code{goalkeeping\_mean} feature helps separate goalkeeper profiles from outfield-player profiles, allowing one model to handle both groups.
...
If a clean, independently sourced version of the \code{potential} feature becomes available, it should be added through the feature store and treated as a new model version, with leakage checks and retraining before use.

% ===========================================================================
\section{Environment \& Dependencies}
% ===========================================================================
The environment is declared in \code{pyproject.toml}. Rather than listing every transitive package, here are the main packages used by the pipeline:
...
\begin{itemize}
    ...
    \item \textbf{Modelling \& Metrics:} \code{scikit-learn} (1.x) for candidate models and preprocessing, with \code{scipy} (1.1x) backing the statistical drift tests.
    ...
    \item \textbf{Testing:} \code{pytest} runs our 30 unit and integration tests.
\end{itemize}
