...
We chose this dataset because it forces us to handle common data engineering headaches: missing values, messy categorical fields, date parsing, and a leaky column that practically gives away the target. After cleaning, the valid \code{overall\_rating} values range from 42 to 90. To keep development cycles fast, we run the pipeline on a fixed 6,000-row sample (seed 42) so the whole thing executes in a couple of minutes. The full table can be built on demand with \code{scripts/build\_dataset.py}.
...
% ===========================================================================
Instead of dumping everything into a single Jupyter notebook, we broke the project down into separate Kedro pipelines to mimic a production environment. Over six weeks, we implemented data quality checks, cleaning, a feature store, model training with MLflow tracking, explainability, containerized serving, and finally drift monitoring. By doing this, each component operates as a standalone, runnable unit.

We structured the data using Kedro's standard layering (\code{01\_raw} to \code{08\_reporting}). As shown in Figure~\ref{fig:dag}, the architecture consists of eight interconnected pipelines. The data prep stages run sequentially, followed by two parallel branches that use the trained champion model. Running \code{kedro run} executes the entire DAG, but we can also trigger individual components with \code{kedro run -{}-pipeline <name>}. This modular setup reflects how the system would run in production: \code{model\_predict} can score new data, and \code{data\_drifts} can check for distribution shifts without needing to retrain the model from scratch.
...
calls the same \code{preprocess\_for\_inference} routine as the offline pipeline, so the two paths
can't drift apart. We'd tighten this further with a row-level test that checks both paths produce
...
Initial EDA (\code{exploratory\_data\_analysis.ipynb}) on the raw sample informed several preprocessing steps. As shown in Figure~\ref{fig:eda}, the target \code{overall\_rating} is roughly symmetric and bounded, so we modeled it directly without transformation. 
...
The \code{model\_train} node generates feature importance metrics using SHAP (falling back to permutation importance if necessary). Figure~\ref{fig:shap} highlights the most impactful features.
...
The SHAP summary plot confirms \code{reactions} and \code{ball\_control} are the primary drivers, consistent with our initial EDA. The \code{goalkeeping\_mean} feature effectively bifurcates predictions, allowing a single model to score both field players and goalkeepers.
...
The baseline run (\code{drift\_report.json}) flagged 1 out of 48 features. With $\alpha=0.05$, a single flag is within expected false-positive bounds, so no dataset-level drift was triggered. To validate the monitoring logic, we simulated a drift scenario (\code{drift\_report\_simulated.json}), which successfully triggered flags on 18 features and raised a dataset-level alert. Figure~\ref{fig:drift} compares the PSI scores between the clean and drifted batches. Currently, this monitors feature drift; target and concept drift monitoring are planned for future iterations.
...
% ===========================================================================
Right now, the pipeline runs locally on a static CSV sample. To transition this from a proof of concept to a real production system, we'd need to upgrade a few pieces of the stack. Following standard engineering practices, we've organized these into a "Now/Next/Later" roadmap.

\textbf{Next (Immediate upgrades):}
\begin{itemize}
    \item \textbf{Orchestration \& Scheduling:} The Kedro CLI is great for testing, but we need to automate runs. We plan to deploy this via \code{kedro-airflow} so we have proper scheduling and retries.
    \item \textbf{Tracking Server:} Our MLflow setup currently uses a local SQLite file. We need to migrate this to a hosted MLflow server with an object-store backend (like S3) to allow for multi-user collaboration.
...
\begin{itemize}
    \item \textbf{Distributed Compute:} Pandas handles our 184k-row table fine for now, but as the dataset grows, we should port the pipeline nodes to Spark or Dask.
...
As a side note, if a clean, independently sourced version of the \code{potential} feature ever becomes available, the feature store would be the ideal place to enforce rules to add it back without breaking the current model.

% ===========================================================================
\section{Environment \& Dependencies}
% ===========================================================================
To keep the environment fully reproducible, we pinned all \raisebox{0.5ex}{\~}250 dependencies in \code{requirements-lock.txt}. Rather than listing every package, here is the core tech stack that does the heavy lifting:
